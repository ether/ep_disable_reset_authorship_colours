# Contributing to ep_disable_reset_authorship_colours

Thanks for helping improve `ep_disable_reset_authorship_colours` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_disable_reset_authorship_colours` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_disable_reset_authorship_colours from your local clone
pnpm run plugins i --path ../ep_disable_reset_authorship_colours

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_disable_reset_authorship_colours/` are picked up after a server restart.
...
* Etherpad version, ep_disable_reset_authorship_colours version, browser + OS
* Steps to reproduce
